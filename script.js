        ...
        exportButtons.forEach(button => {
            ...
            newBtn.addEventListener('click', () => {
                ...
                // >>> C. PDF 下载 (修复版：移除滚动条，强制自然高度) <<<
                    ...
                    // 2. 创建临时容器
                    // 🔴 关键修改：使用 absolute 定位，取消 height 限制，取消 scroll
                    const container = document.createElement('div');
                    container.style.position = 'absolute'; // 改为 absolute，随文档流
                    container.style.left = '0';
                    container.style.top = '0';
                    container.style.width = '100%';
                    // container.style.height = '100%'; // ❌ 删除这行，不要限制高度
                    // container.style.overflowY = 'scroll'; // ❌ 删除这行，禁止滚动
                    container.style.height = 'auto'; // ✅ 让内容自然撑开
                    ...
                    // 模拟 A4 排版样式
                    container.innerHTML = `
                        <div style="max-width: 800px; margin: 0 auto; font-family: Helvetica, Arial, sans-serif; color: #333;">
                            <div style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #007bff; padding-bottom: 20px;">
                                <h1 style="color: #007bff; margin: 0; font-size: 26px;">Professional Report</h1>
                                <p style="color: #666; font-size: 14px; margin-top: 5px;">Generated by Reportify AI • ${dateStr}</p>
                            </div>
                            <div style="font-size: 14px; line-height: 1.6; text-align: left;">
                                ${htmlContent}
                            </div>
                            <div style="margin-top: 40px; text-align: center; font-size: 10px; color: #aaa;">
                                - End of Document -
                            </div>
                        </div>
                    `;
                    ...
                    // 插入到 body 最顶端，覆盖一切
                    document.body.appendChild(container);
                    window.scrollTo(0, 0); // 强制滚回到顶部，防止截图截歪
