...
    try {
      // (!!!) 修复点 1: 
      // 您的后端 API (根据我们的 index.js) 是 /api/me, 而不是 /api/user/profile
...
      
      // (!!!) 修复点 2: 
      // 您的后端 (根据我们的 index.js) 发送的是 'displayName', 而不是 'name'
...
           S_C_R_I_P_T_ _E_N_D_       console.error('Error calling generate API:', error);
                if (resultBox) {
...
                } else if (format === 'Word') {
                    try {
          _B_L_ / _B_L_                 // Ensure docx is loaded
                        if (typeof docx === 'undefined') {
                            alert("Word export library not loaded. Please refresh.");
S_C_R_I_P_T_ _E_N_D_                         console.error("docx library is not defined.");
                            return;
                        }
...
  S_C_R_I_P_T_ _E_N_D_     const data = await res.json();
...
            } finally {
              _B_L_ / _B_L_ submitBtn.disabled = false;
                submitBtn.textContent = originalBtnText;
            }
        });
    }
...
S_C_R_I_P_T_ _E_N_D_                           openModal('login');
...
S_C_R_I_P_T_ _E_N_D_                         // Example backend call:
...
                        });
                    },
S_C_R_I_P_T_ _E_N_D_                 onError: function(err) {
