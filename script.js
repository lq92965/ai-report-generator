        ...
        exportButtons.forEach(button => {
            ...
            newBtn.addEventListener('click', () => {
                ...
                // >>> C. PDF 下载 (终极修复：遮罩层强制渲染) <<<
                    ...
                    // 2. 创建一个临时的、位于屏幕最顶层的容器
                    // 必须让它在视口内可见，html2pdf 才能截图成功
                    const container = document.createElement('div');
                    container.style.position = 'fixed';
                    container.style.top = '0';
                    container.style.left = '0';
                    container.style.width = '100%';   // 占满全屏
                    container.style.height = '100%';
                    container.style.zIndex = '99999'; // 只有这样才能盖住其他元素
                    container.style.background = '#fff'; // 白底
                    container.style.overflowY = 'scroll'; // 允许滚动
                    container.style.padding = '40px';
                    container.style.boxSizing = 'border-box';
                    
                    // 模拟 A4 纸张的排版
                    container.innerHTML = `
                        <div style="max-width: 800px; margin: 0 auto; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333;">
                            <div style="text-align: center; margin-bottom: 40px; border-bottom: 2px solid #007bff; padding-bottom: 20px;">
                                ...
                                <p style="color: #666; font-size: 14px; margin-top: 10px;">Generated by Reportify AI • ${dateStr}</p>
                            </div>
